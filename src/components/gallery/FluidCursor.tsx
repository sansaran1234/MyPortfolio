...
export type FluidCursorProps = {
  ...
  /** Blob fill color */
  color?: string;
  /** Base opacity for the lead blob */
  opacity?: number;
  /** Follow speed — higher = snappier leader */
  speed?: number;
  /** Trail elasticity — higher = tighter trailing */
  elasticity?: number;
};
...
export const FluidCursor = ({
  ...
  const [blobs, setBlobs] = useState<Point[]>(() =>
    Array.from({ length: trailCount + 1 }, () => ({ x: -120, y: -120 })),
  );

  const targetRef = useRef<Point>({ x: -120, y: -120 });

  useEffect(() => {
    setBlobs(Array.from({ length: trailCount + 1 }, () => ({ ...targetRef.current })));
  }, [trailCount]);
    ...
    const tick = () => {
      setBlobs((previous) => {
        ...
        for (let index = 1; index < next.length; index += 1) {
          const trailEase = elasticity + index * 0.035;
          next[index] = {
  ...
  return (
    <div aria-hidden className="fluid-cursor-root">
      {blobs.map((blob, index) => {
        ...
        const blobSize = size * (0.55 + (trailIndex / Math.max(trailCount, 1)) * 0.45);
        const blobOpacity = opacity * (0.22 + (trailIndex / Math.max(trailCount, 1)) * 0.78);
        const scale = isLead
          ? hovering
            ? 1.45
            : 1
